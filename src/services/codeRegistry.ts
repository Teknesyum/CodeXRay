...
const algorithmPresets: AlgorithmPreset[] = [
  ...
  {
    "id": "longest-palindromic-substring-manacher-s",
  ...
  {
    "id": "fast-exponentiation-modular",
...
const normalizePresetQuery = (value: string): string => value
...
const presetAliases: Record<string, string> = {
  ...
  'z algoritması': 'z-algorithm-string-matching',
  'z-algorithm': 'z-algorithm-string-matching',
  'kmp': 'kmp-string-matching',
  'rabin karp': 'rabin-karp-string-matching',
  'trie': 'trie-insert-and-search',
  'segment tree': 'segment-tree-range-sum',
  'fenwick tree': 'fenwick-tree-binary-indexed-tree',
  'disjoint set': 'disjoint-set-union-find',
  'lca': 'lowest-common-ancestor-lca',
};

const presetsById = new Map(algorithmRegistry.map((preset) => [preset.id, preset]));
const commandAliases = [
  ...Object.entries(presetAliases).map(([alias, id]) => ({
    alias: normalizePresetQuery(alias),
    id,
  })),
  ...algorithmRegistry.flatMap((preset) => [
    { alias: normalizePresetQuery(preset.name), id: preset.id },
    { alias: normalizePresetQuery(localizeAlgorithmName(preset.name, 'tr')), id: preset.id },
  ]),
];

export const resolveAlgorithmPresetById = (id: string): AlgorithmPreset | undefined =>
  presetsById.get(id);

export const resolveAlgorithmPreset = (query: string): AlgorithmPreset | undefined => {
  const directId = resolveAlgorithmPresetById(query.toLocaleLowerCase('en-US').trim());
  if (directId) return directId;

  const normalized = normalizePresetQuery(query);
  const aliasId = presetAliases[normalized];
  return aliasId ? resolveAlgorithmPresetById(aliasId) : undefined;
};

export const resolveAlgorithmPresetFromCommand = (
  command: string,
): AlgorithmPreset | undefined => {
  const normalized = ` ${normalizePresetQuery(command)} `;
  ...
  return resolveAlgorithmPresetById([...matchingIds][0]);
};
