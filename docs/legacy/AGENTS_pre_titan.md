# AGENTS.md before Titan (archived)

Archived on 2026-08-24 during the Titan handover protocol rewrite. The body below is the
verbatim 204-line pre-Titan root `AGENTS.md`. Nothing in it is authoritative any more; the
live guide is the repository root `AGENTS.md`.

## What was removed and why

- `## Handoff Record` (lines 10-26) — removed from the root guide; the whole handoff
  mechanism moved to `docs/titan/PROTOCOL.md` and `docs/titan/handoffs/H*.md`.
  `docs/DEVIRALAN.md` is now a periodic rollup, not the live channel. A one-line pointer
  remains in the new guide.
- `godModeOrchestrator.ts` architecture entry (lines 111-113) — the file no longer exists;
  it was deleted in T18. Keeping the entry would send agents looking for dead code.
- God Mode contract paragraph (lines 180-188) — God Mode was retired as a named mode. Its
  surviving invariants (schema validation, no raw model text, audit snapshot, undo and
  rollback, no filesystem/OS/credential/network authority) were rewritten under the Titan
  five-phase pipeline in the new `AI and trace contracts` section.
- `webSource.ts` / `webProblemOrchestrator.ts` architecture entry (lines 114-118) —
  shortened from 8 lines to 3. The removed detail (ManagerPlanV2 job wording, Java 17
  fallback mechanics, critic gate internals) is derivable from the source; the security
  boundary sentence was kept.
- WebLLM origin and OPFS/Cache section (lines 66-85) — shortened from 20 lines to 5. The
  removed detail was operational narrative about hot reload and cross-tab locking. The
  load-bearing sentence "never clear the whole origin" was kept verbatim in meaning.
- `## Commands` block (lines 30-38) — replaced with the full real script list from
  `package.json`, which the old block did not cover (`test:watch`, `test:e2e:ai`,
  `test:e2e:radio-live`, `desktop:*`, `publish:site`).
- Ordinary prose compression throughout `## Architecture` and `## Contracts` — every rule
  was carried into the new sectioned layout; no invariant was dropped. Only sentence
  length changed.

Nothing else was deleted. Everything preserved verbatim in meaning: the Windows Playwright
external-server procedure and its PID cleanup rule, the `eval` / `new Function` ban,
determinism, EN+TR localization, the `isSupported: true` precondition, the `TraceValue`
truncation ban, `lineNumber` 1-based-or-null, GraphDocumentV1 validation, atomic node ID
updates, pin ordering, panel and splitter rules, the `siteReset` boundary, the
`publish:site` guard, and the build-artifact commit ban.
