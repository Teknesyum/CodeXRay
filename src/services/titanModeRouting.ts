...
export const routeWebSourceRequest = (
  ...
  const text = normalizeTitanModeText(textWithoutUrl);
...
export const normalizeTitanModeText = (value: string): string => value
...
export const extractDpDimensions = (request: string): { rows: number; columns: number } | null => {
  const match = normalizeTitanModeText(request).match(/\b(\d{1,2})\s*(?:x|\*)\s*(\d{1,2})\b/);
...
export const requestsUniqueDpInput = (request: string): boolean => {
  const text = normalizeTitanModeText(request);
...
export const canonicalCustomTitle = (request: string, locale: Locale): string => {
  const normalized = normalizeTitanModeText(request);
...
export const routeTitanModeRequest = (
  question: string,
  steps: SimulationStep[],
  currentIndex: number,
  algorithmName = '',
): TitanModeIntent | null => {
